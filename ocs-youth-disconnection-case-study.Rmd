...
df2 <- df2 %>% 
  mutate(Type = case_when(is.na(Gender) & is.na(Subgroup) ~ "US Total",
...
```{r}
image3 <- image_read(here("latino_subgroups.png"))
```
...
df3 <- df3 %>% 
  mutate(Type = case_when(is.na(Gender) & is.na(Subgroup) ~ "Latino Total",
                          is.na(Gender) & !is.na(Subgroup) ~ "Subgroup Total",
                          !is.na(Gender) & is.na(Subgroup) ~ "Gender Total",
                          TRUE ~ "Subgroup Total"))
...
```{r}
image5 <- image_read(here("latino_subgroups_2018.png"))
...
df2 %>%
  ...
  geom_bar(stat = "identity",
           ...
           position = "dodge",
           width = 0.5) +
  labs(title = "FIGURE 2 YOUTH DISCONNECTION BY ASIAN SUBGROUP, 2017",
...
df3 %>%
  ...
  mutate(Subgroup = fct_reorder(Subgroup, rank_all)) %>%
  ggplot(aes(x = Subgroup, y = Rate, fill = Gender)) +
  geom_hline(yintercept = latino_total, 
             color = "black",
             linetype = 2) + 
  geom_bar(stat = "identity",
           color = "transparent",
