...
We are particularly interested in being able to reproduce the plot below, as we would like to make similar plots but include gender subgroups.
...
Now let's create a similar plot for females:
...
plot_gender <- directlabels::direct.label(plot_gender, list(dl.trans(x = x+0.3, y = y +0), "last.points"))
```
...
It's a bit difficult to tell the Black and Latinx female lines apart. 

Maybe we can add another color for every other line. 
```{r}
...
We also want to make plots of the Asain and Latinx subgroups. 
We can make our plots look like they match the report by using a custom color palette to create a gradient based off the color  we just used. 
...
Then we just need to figure out how many groups we have. In this case we have 3 gender groups.
...
Then when we use the palette we will do this to specify how many groups we have. The `rev()` function revereses the order.
```{r, eval = FALSE}
scale_color_manual(values = rev(custom_pal(major_group_n + 1)))
```



We will first use this to create a plot for the asain subgroups of the 2017 data.
