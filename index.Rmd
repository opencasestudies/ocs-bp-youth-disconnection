...
youth_plot <- directlabels::direct.label(youth_plot, list(dl.trans(x = x+1.2, y = y +0), "last.points"))
```
...
```{r, eval = FALSE}
(plot_spacer() + plot_by_gender)/ (asian_subgroup_plot_simp)

```

The `plot_layout()` function allows for specification of the relative heights and widths of the various plot elements. We  will first use this function to specify the relative widths of  blank spaces on either side of the `plot_by_gender` plot. Then, since we will create three rows (distinguished by the `/` symbols, we need to specify widths and heights for all 3 rows.)

AVOCADO: just a reminder I removed `asian_gender_subgroup_plot` and `latinx_gender_subgroup_plot` from the code above so plot below needs to updated. 

```{r, eval = FALSE}
  (plot_spacer() + plot_by_gender + plot_spacer() + 
...
AVOCADO: I set the code chunk below to eval=FALSE because I removed `asian_gender_subgroup_plot` and `latinx_gender_subgroup_plot` from the code above.
