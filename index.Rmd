...
[tidyverse](https://readr.tidyverse.org/)      | for data science operations
[pdftools](https://readr.tidyverse.org/)      | to manage PDF documents
[magick](https://cran.r-project.org/web/packages/magick/vignettes/intro.html#Kernel_convolution)      | for image processing 
[here](https://github.com/jennybc/here_here)       | to easily load and save data
[tidyverse](https://readr.tidyverse.org/)      | for data science operations
[pdftools](https://readr.tidyverse.org/)      | to manage PDF documents
[magick](https://cran.r-project.org/web/packages/magick/vignettes/intro.html#Kernel_convolution)      | for image processing 
...
See [here](https://youth.gov/evidence-innovation/program-directory?keywords=&field_pd_factors_risks_tid=413&field_pd_factors_protective_tid=All) and [here](https://goc.maryland.gov/wp-content/uploads/sites/8/2015/10/Program-Models-for-Serving-Opportunity-Youth.pdf) for listings of programs dedicated to rengaging disconnected youth or preventing disconnection.

See [here](https://www.communitiesthatcare.net/) and 
[here](https://extension.psu.edu/promoting-school-community-university-partnerships-to-enhance-resilience) for particular examples.
...
```{r}
# custom_pal <- colorRampPalette(c("navy", "white"))
# custom_pal(3)
# 
...
The Mann-Kendall trend test is a non-parametric test, which means that it does not require as many assumptions as some parametric tests, like the [simple linear regression](https://en.wikipedia.org/wiki/Simple_linear_regression).
...
$ sign(x_{j}-x_{k}) = 1$ if $x_{j}-x_{k} >0$
                   $= 0$ if $x_{j}-x_{k} = 0$
                   $= -1$ if $x_{j}-x_{k} <0 $
...
$t_{p} = $ number of observations in the $p$th group for a given tie
...
Finally the $Z_{MK}$ statistic would be calculated as (since $S$ < 0):
...
We can also accomplish this with the `Kendall::MannKendall()` function. The `Kendall::MannKendall()` accepts a vector of data for which a trend may be observed. [Consulting the documentation for the `Kendall::MannKendall()` function available on CRAN](https://rdrr.io/cran/Kendall/man/MannKendall.html), we can "test for a a monotonic trend in a time series".
...
```
Again, we see that the score $S$ is -7. The larger the score, the more indication that there is an increasing or decreasing monotonic relationship. We also again see that the pvalue is roughly = 0.26 and is much greater than 0.05, suggesting that we don't see a trend in our data.  
...
However, it's important to note that we only have `r length(major_groups_long %>% filter(Gender == "All", Race_Ethnicity == "Native American") %>% pull(Percent))` observations.
...
Indeed this data does show a trend toward declining rates of youth disconnection as the score is again negative and the p value is now  much less than 0.5 for even the 2-sided hypothesis. 
...
[Different types of regression](https://www.analyticsvidhya.com/blog/2015/08/comprehensive-guide-regression/)  
[Ordinary least squares method](http://setosa.io/ev/ordinary-least-squares-regression/)  
[Residual](https://www.statisticshowto.datasciencecentral.com/residual/)  
