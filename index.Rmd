...
asian_total
US_total
asian_total["Gender"]<-c("Female")
US_total["Gender"]<-c("Female")
```
...
# geom_point(aes(fill = Gender), size = 2, position = position_jitterdodge(jitter.width = 0), shape = 4 )+
  labs(title = "YOUTH DISCONNECTION BY ASIAN SUBGROUP AND GENDER, 2017-2018",
       y = "YOUTH DISCONNECTION (%)") +
  ...
  labels = c("Asian" = "Asian Avg.",
   ...
   scale_y_continuous(limits = c(0,max(pull(asian_for_plot,Percent), 
                                       na.rm = TRUE)))+
  bar_theme()+
  theme(legend.justification = c(0.18,0.05), legend.position = c(0.18,.05))+
   geom_text(data = asian_total,
         mapping = aes(x = 3, y = Percent +.5, label = label),
           parse = TRUE,
           color = "#008393",
            size = 3) +
  geom_hline(aes(yintercept = pull(asian_total, Percent)), linetype = 2) +
...
ggplot(aes(x = Race_Ethnicity, y = Percent)) +
geom_col(aes(fill = Gender), position = "dodge")+
  labs(title = "YOUTH DISCONNECTION BY LATINX SUBGROUP AND GENDER, 2017-2018",
       y = "YOUTH DISCONNECTION (%)") +
  scale_fill_manual(values = gender_pal) +
  scale_x_discrete(
  limits = c(levels(pull(latinx_for_plot,Race_Ethnicity))[1],
  ...
  labels = c("Latinx" = "Latinx Avg.",
             "Blank" = "",
             "Next_Blank" = ""))+
   scale_y_continuous(limits = c(0,max(pull(latinx_for_plot,Percent), 
                                       na.rm = TRUE)))+
...
ggplot(aes(x = Race_Ethnicity, y = Percent)) +
 geom_col(aes(fill = Gender), position = "dodge")+
  ...
  labs(title = "ASIAN SUBGROUP YOUTH DISCONNECTION BY GENDER, 2017-2018",
       y = "YOUTH DISCONNECTION (%)") +
  scale_fill_manual(values = gender_pal) +
  scale_x_discrete(
  limits = c(levels(pull(asian_for_plot,Race_Ethnicity))[1],
...
ggplot(aes(x = Race_Ethnicity, y = Percent, fill = Year)) +
 geom_col(aes(fill = Year), position = "dodge")+
  facet_wrap(~Gender, scales = "free") +
  labs(title = " LATINX SUBGROUP YOUTH DISCONNECTION BY GENDER, 2017-2018",
       y = "YOUTH DISCONNECTION (%)") +
  scale_fill_manual(values = (custom_pal(4))) +
  scale_x_discrete(
  limits = c(levels(pull(latinx_for_plot,Race_Ethnicity))[1],
...
plot_gender / (asian_subgroup_plot_simp + asian_gender_subgroup_plot) /(latinx_subgroup_plot_simp + latinx_gender_subgroup_plot)
dev.off()
```
