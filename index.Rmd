...
Great! Our function works, and this looks much better!
...
We also want to add a couple of variables about `Race` and `Gender` so that we can select across groups later.  We can use the `recode()` function of the `dplyr` package to change specific values, as we create a new `Race` variable from the `Group` variable. For the Data for all of the US we want the `Race` variable values to be `"All_races"`.
...
We also want to remove Male and Female from this "Race" variable, We can do so using the `str_remove()` function of the `stringr` package. Importantly, we are also removing the space beore "Female" and "Male.
...
```{r}
major_groups %<>%
  mutate(across(.cols = c(Group, Race), 
                ~str_replace(string = ., 
...
```{r}
latino_a_sub_2017 <-stringr::str_c(Latino_a_A, Latino_a_B, Latino_a_C)
```
...
```{r}
latino_a_sub_2017 <- clean_table(table = latino_a_sub_2017)
latino_a_sub_2017
```
...
```{r, echo = FALSE}
library(DT)
DT::datatable(latino_a_sub_2017)
```
...
```{r}
knitr::include_graphics(here::here("img", "latino_a_youth_excerpt.png"))
...
latino_a_subgroups <- bind_rows(latino_a_sub_2017, latino_a_sub_2018)
latino_a_subgroups
```
...
save(major_groups_long, asian_subgroups, latino_a_subgroups,
...
There does not appear to be a change in the trend. However, it's important to note that we only have `r length(major_groups_long %>% filter(Gender == "All", Race == "Native American") %>% pull(Percent))` observations.
...
`r (major_groups_long%>% filter(Gender == "All", Race == "Native American") %>% lm(Percent ~ Year, data = .))[["coefficients"]]["Year"]`
...
asian_subgroups %>%
  ...
  ungroup() %>%
  group_by(Year) %>%
  mutate(threshold = Percent[Gender == "ALL" & Subgroup == "ALL"]) %>%
  ungroup() %>%
  mutate(Subgroup = fct_reorder(Subgroup, rank_all)) %>%
  ggplot(aes(x = Subgroup, y = Percent, fill = Gender)) +
