...
library(readr)
library(magrittr)
required_packages <- c("wordcountaddin", "read.so", "koRpus", "koRpus.lang.en")
...
```{r, echo=FALSE}
readability_text <- getFromNamespace("prep_text", "wordcountaddin")(
  paste(scan("index.Rmd", "character", quiet = TRUE), collapse = " ")
)
suppressPackageStartupMessages(require("koRpus.lang.en", quietly = TRUE))
readability_words <- strsplit(readability_text, " ", fixed = TRUE)[[1]]
readability_words <- ifelse(
  nchar(readability_words) > 30,
  substr(readability_words, 1, 10),
  readability_words
)
readability_tokens <- koRpus::tokenize(
  readability_words,
  format = "obj",
  lang = "en"
)
invisible(
  capture.output(
    readability_result <- suppressMessages(koRpus::readability(
      readability_tokens,
      index = c("Flesch.Kincaid", "FORCAST", "SMOG"),
      quiet = TRUE
    ))
  )
)
df <- summary(readability_result) %>%
  dplyr::mutate(
    grade = round(as.numeric(grade)),
...
The data used in these reports comes from the [American Community Survey (ACS)](https://www.census.gov/programs-surveys/acs), which is the largest survey conducted by the United States Census Bureau. The survey started in 2005 and collects data for 3.5 million households annually. Data is collected about ancestry, citizenship, income, employment, disability among many other aspects. See [here](https://en.wikipedia.org/wiki/American_Community_Survey) for more detailed information about the survey.
...
raw_data("ocs-bp-youth-disconnection", outpath = getwd())
...
write_file(major_groups, file = here::here("data", "simpler_import", "major_groups.csv"))
write_file(asian_sub_2017, file = here::here("data", "simpler_import", "asian_sub_2017.csv"))
write_file(asian_sub_2017_A, file = here::here("data", "simpler_import", "asian_sub_2017_A.csv"))
write_file(asian_sub_2017_B, file = here::here("data", "simpler_import", "asian_sub_2017_B.csv"))
write_file(asian_sub_2017_C, file = here::here("data", "simpler_import", "asian_sub_2017_C.csv"))
write_file(latinx_sub_2017_A, file = here::here("data", "simpler_import", "latinx_sub_2017_A.csv"))
write_file(latinx_sub_2017_B, file = here::here("data", "simpler_import", "latinx_sub_2017_B.csv"))
write_file(latinx_sub_2017_C, file = here::here("data", "simpler_import", "latinx_sub_2017_C.csv"))
write_file(asian_sub_2018_A, file = here::here("data", "simpler_import", "asian_sub_2018_A.csv"))
write_file(asian_sub_2018_B, file = here::here("data", "simpler_import", "asian_sub_2018_B.csv"))
write_file(latinx_sub_2018, file = here::here("data", "simpler_import", "latinx_sub_2018.csv"))
```
...
If you have been following along but stopped and are starting here you could load the imported data using the following command:
...
```{r, eval=FALSE}
# library(OCSdata)
imported_data("ocs-bp-youth-disconnection", outpath = getwd())
...
We can use the assignment arrow (`<-`) to save the transformed `major_groups` data back to the same object. This makes the input and reassignment explicit.
...
Then, you may load the wrangled data into your R environment using the following code:

```{r, eval=FALSE}
# library(OCSdata)
wrangled_rda("ocs-bp-youth-disconnection". outpath = getwd())
...
To add the figurines we can use icons from free resources like [Font Awesome](https://fontawesome.com/). See [here](https://commons.wikimedia.org/wiki/Category:Font_Awesome_4_icons) for `svg` options. We include a local copy of the user icon with this case study, so the example can be rendered without downloading an image. Or we could also take a screenshot of just the figurine from the original plot. However, it is useful to know how to create similar icons if we didn't have the original plot to work with, so we will start with an example using a Font Awesome icon.


##### Using font awesome:
First we import the local copy of the Font Awesome image:

```{r, out.width="10%"}

# Use the checked-in copy so rendering does not depend on Wikimedia or DNS.
fa_figurine <- image_read(here::here("img", "user_font_awesome.svg"))
...
The data in these reports comes from the [American Community Survey(ACS)](https://www.census.gov/programs-surveys/acs).
