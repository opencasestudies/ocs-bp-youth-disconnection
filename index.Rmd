...
[readr](https://readr.tidyverse.org/) | for saving files
...
knitr::include_graphics(here::here("www", "img", "jon-tyson-ajzN2AYNi1U-unsplash.jpg"))
```
...
knitr::include_graphics(here::here("www", "img", "risk_factors.png"))
```
...
```{r, out.width="400px", echo = FALSE}
knitr::include_graphics(here::here("www", "img", "Making_the_Connection.png"))
```
...
```{r, out.width="400px", echo = FALSE}
knitr::include_graphics(here::here("www", "img", "A_Decade_Undone.png"))
```
...
knitr::include_graphics(here::here("www", "img", "gender_race_ethnicity_overview.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "latinx_2018_overview.png"))
```

```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "asian_2018_overview.png"))
```
...
If you did not clone the GitHub repository, you may download the raw data files, including the pdf file of the report and the screenshots, using the `OCSdata` package:

```{r, eval=FALSE}
# install.packages("devtools")
# devtools::install_github("opencasestudies/OCSdata")

load_raw_data("ocs-bp-youth-disconnection", outpath = getwd())
# This function creates a "data" sub-directory in your current working directory.
# It also creates a "raw" sub-directory within "data" which contains the .pdf file.
```

### **Importing with `pdftools`**
***

If you still have trouble obtaining the pdf file, you can download it using this [link](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/raw/Making_the_Connection.pdf).

One way to import data from a pdf is to use the `pdf_text()` function of the `pdftools` package. In our case, we downloaded the `Making_the_Connection.pdf` file and put it within a `raw` subdirectory of a `data` directory for our project. If you use an RStudio project, then you can use the `here()` function of the `here` package to specify the path or location of the document that we want to import. The `here` package automatically starts looking for files based on where you have a `.Rproj` file which is created when you start a new RStudio project. We can specify that we want to look for the `Making_the_Connection.pdf` file within the `raw` directory within the `data` directory within a directory where our `.Rproj` file is located. Starting from "data", we separate the names of these directories using commas.
...
*Note the next chunk of code will only work for you if you set up your file structure in the same way as stated above.*

```{r, eval=FALSE}
pdf_tools_example <- 
  pdftools::pdf_text(here::here("data", "raw", "Making_the_Connection.pdf"))
```

```{r, echo=FALSE}
pdf_tools_example <- 
  pdftools::pdf_text(here::here("www", "data", "raw", "Making_the_Connection.pdf"))
```
...
From the output, it's clear that a relatively large amount of manipulation will be required to wrangle this data. If you are interested in learning more about the `pdftools` package, please see this [case study](https://opencasestudies.github.io/ocs-bp-rural-and-urban-obesity/) and this [case study](https://opencasestudies.github.io/ocs-bp-diet/).


#### {.think_question_block}
<b><u> Question Opportunity </u></b>

If you are familiar with `pdftools`, spend 3-5 minutes trying to extract the multiple tables from page 44. 

####

```{r, echo=FALSE}
save(pdf_tools_example, file = here("www", "exercise", "di_code1.rda"))
```
...
load(here("www", "exercise", "di_code1.rda"))
```

```{r di_code1, exercise=TRUE}
# try this out if you'd like
```
...
knitr::include_graphics(here::here("data", "raw", "Major_ethnic_groups_screenshot.png"))
```

```{r, out.width="60%", echo=FALSE}
knitr::include_graphics(here::here("www", "data", "raw", "Major_ethnic_groups_screenshot.png"))
```

You can download this image file from our [GitHub repostory](https://github.com//opencasestudies/ocs-bp-youth-disconnection) to follow along [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/raw/Major_ethnic_groups_screenshot.png).
...
```{r, dpi = 1000, eval=FALSE}
Major_racial_ethnic_groups <- 
  magick::image_read(here::here("data", "raw", "Major_ethnic_groups_screenshot.png"))

magick::image_info(Major_racial_ethnic_groups)
```

```{r, dpi = 1000, echo=FALSE}
Major_racial_ethnic_groups <- 
  magick::image_read(here::here("www", "data", "raw", "Major_ethnic_groups_screenshot.png"))
...
OK! So now let's try extracting the text for our `Major_racial_ethnic_groups` image.

### **Magor Racial and Ethnic Groups**
...
Excellent, now we need to do the same for the other two tables on the bottom half of this page that we are interested in:

```{r, echo  = FALSE}
knitr::include_graphics(here::here("www", "img", "gender_race_ethnicity_overview.png"))
```

So now we will take some screenshots of the other tables of interest. **We will later explain why we took the specific screenshots that we did, but for now just focus on importing the data.** In general,it is helpful to leave off any additional header information that you don't intend to include in the actual table. In addition empty spaces in tables can cause the need for additional images.

### **Asian Subgroups 2017**
***
First, we are also interested in data about the Asian subgroups in 2017.

If you have not done so already, you can download the next image file [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/raw/asian_subgroups_2017.png).
...
```{r, eval=FALSE}
asian_sub_2017 <- image_read(here::here("data", "raw", "asian_subgroups_2017.png"))
```

```{r, echo=FALSE}
asian_sub_2017 <- image_read(here::here("www", "data", "raw", "asian_subgroups_2017.png"))
```
...
Trial and error during the wrangling section will demonstrate that we needed some additional images. We will import these now. 

If you have not done so already, you can download the next image files to follow along [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/raw/asian_sub_2017_A.png) and [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/raw/asian_sub_2017_B.png) and [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/raw/asian_sub_2017_C.png).

```{r, out.width= "40%", eval=FALSE}
asian_sub_2017_A <- image_read(here("data", "raw", "asian_sub_2017_A.png"))
asian_sub_2017_B <- image_read(here("data", "raw", "asian_sub_2017_B.png"))
asian_sub_2017_C <- image_read(here("data", "raw", "asian_sub_2017_C.png"))
...
asian_sub_2017_A <- image_read(here("www", "data", "raw", "asian_sub_2017_A.png"))
asian_sub_2017_B <- image_read(here("www", "data", "raw", "asian_sub_2017_B.png"))
asian_sub_2017_C <- image_read(here("www", "data", "raw", "asian_sub_2017_C.png"))
...
OK, now moving on to the last table on this page

### **Latinx Subgroups 2017**

```{r, out.width = "70%", echo = FALSE}
knitr::include_graphics(here::here("www", "img", "latinx2017.png"))
```

Again after trial and error, three screenshots were determined best for importing this data. 

The names of the files for the screenshots are `latinx_sub_2017_A.png`, `"latinx_sub_2017_B.png`, and `"latinx_sub_2017_C.png"`. They are located in the "raw" subdirectory of the "data" directory in our GitHub repository.

If you have not done so already, you can download the next image files to follow along [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/raw/latinx_sub_2017_A.png) and [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/raw/latinx_sub_2017_B.png) and [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/raw/latinx_sub_2017_C.png).
...
```{r, eval=FALSE}
latinx_imageA <- image_read(here::here("data", "raw", "latinx_sub_2017_A.png"))
latinx_imageB <- image_read(here::here("data", "raw", "latinx_sub_2017_B.png"))
latinx_imageC <- image_read(here::here("data", "raw", "latinx_sub_2017_C.png"))
...
```{r, echo=FALSE}
latinx_imageA <- image_read(here::here("www", "data", "raw", "latinx_sub_2017_A.png"))
latinx_imageB <- image_read(here::here("www", "data", "raw", "latinx_sub_2017_B.png"))
latinx_imageC <- image_read(here::here("www", "data", "raw", "latinx_sub_2017_C.png"))
...
Great! Now let's import the text from some images for the 2018 data from the other [report](https://ssrc-static.s3.amazonaws.com/moa/ADecadeUndone.pdf).
...
knitr::include_graphics(here::here("www", "img", "asian_subgroups_2018.png"))
```

```{r, echo = FALSE, out.width="80%"}
knitr::include_graphics(here::here("www", "img", "asian_youth_excerpt.png"))
```

Trial and error indicated that again dividing the table into multiple screenshots improved the text extraction:

```{r, eval=FALSE}
asian_sub_2018_A <- image_read(here::here("data", "raw", "asian_sub_2018_A.png"))
asian_sub_2018_A <- image_ocr(asian_sub_2018_A)
asian_sub_2018_B <- image_read(here::here("data", "raw", "asian_sub_2018_B.png"))
asian_sub_2018_B <- image_ocr(asian_sub_2018_B)
```

```{r, echo=FALSE}
asian_sub_2018_A <- image_read(here::here("www", "data", "raw", "asian_sub_2018_A.png"))
asian_sub_2018_A <- image_ocr(asian_sub_2018_A)
asian_sub_2018_B <- image_read(here::here("www", "data", "raw", "asian_sub_2018_B.png"))
asian_sub_2018_B <- image_ocr(asian_sub_2018_B)
```



Finally let's get the Latinx subgroup data for 2018

### **Latinx Subgroups 2018** 
***
Recall that this was the page with the table of interest for the Latinx subgroup 2018 data (published in 2020):
...
```{r, eval=FALSE}
latinx_sub_2018 <- image_read(here::here("data", "raw", "latinx_subgroups_2018.png"))
latinx_sub_2018 <- image_ocr(latinx_sub_2018)
latinx_sub_2018
```

```{r, echo=FALSE}
latinx_sub_2018 <- image_read(here::here("www", "data", "raw", "latinx_subgroups_2018.png"))
latinx_sub_2018 <- image_ocr(latinx_sub_2018)
latinx_sub_2018
```

Excellent! Now let's save all our imported data for users to start at later sections.
...
save(
  ...
  asian_sub_2018_B,
  latinx_sub_2018,
  file = here::here("www", "data", "imported", "imported_data.rda"))
```


```{r, echo = FALSE}
# for instructors who would like to have a simpler data import we will save csv files:

write_file(major_groups, file = here::here("www", "data", "simpler_import", "major_groups.csv"))
write_file(asian_sub_2017, file = here::here("www", "data", "simpler_import", "asian_sub_2017.csv"))
write_file(asian_sub_2017_A, file = here::here("www", "data", "simpler_import", "asian_sub_2017_A.csv"))
write_file(asian_sub_2017_B, file = here::here("www", "data", "simpler_import", "asian_sub_2017_B.csv"))
write_file(asian_sub_2017_C, file = here::here("www", "data", "simpler_import", "asian_sub_2017_C.csv"))
write_file(latinx_sub_2017_A, file = here::here("www", "data", "simpler_import", "latinx_sub_2017_A.csv"))
write_file(latinx_sub_2017_B, file = here::here("www", "data", "simpler_import", "latinx_sub_2017_B.csv"))
write_file(latinx_sub_2017_C, file = here::here("www", "data", "simpler_import", "latinx_sub_2017_C.csv"))
write_file(asian_sub_2018_A, file = here::here("www", "data", "simpler_import", "asian_sub_2018_A.csv"))
write_file(asian_sub_2018_B, file = here::here("www", "data", "simpler_import", "asian_sub_2018_B.csv"))
write_file(latinx_sub_2018, file = here::here("www", "data", "simpler_import", "latinx_sub_2018.csv"))
```
...
Extract the text from the Open Case Studies logo (URL: https://opencasestudies.github.io/img/icon-bahi.png) using the `magick` package.

```{r di_exercise1-setup}
library(tidyverse)
library(magrittr)
library(magick)
```

```{r di_exercise1, exercise=TRUE}
ocs_url <- "https://opencasestudies.github.io/img/icon-bahi.png"
```

```{r di_exercise1-solution}
ocs_url <- "https://opencasestudies.github.io/img/icon-bahi.png"

ocs_logo <- image_read(ocs_url)
...
If you have been following along but stopped and are starting here you could load the imported data using the `OCSdata` package like so:

```{r, eval=FALSE}
# install.packages("devtools")
# devtools::install_github("opencasestudies/OCSdata")
load_imported_data("ocs-bp-youth-disconnection")
```
...
Then, you may load the imported data into your R environment using the following function:

```{r, eval=FALSE}
load_imported_data("ocs-bp-youth-disconnection")
```

If the package does not work for you, an RDA file (stands for R data) of the data can be found [here](https://github.com//opencasestudies/ocs-bp-youth-disconnection/tree/master/data/imported) or slightly more directly [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/imported/imported_data.rda). Download this file and then place it in your current working directory within a subdirectory called "imported" within a subdirectory called "data" to copy and paste our code. We used an RStudio project and the [`here` package](https://github.com/jennybc/here_here) to navigate to the file more easily. 
...
After extracting the text we got this:
...
It takes the object in front of the `%>%` pipe operator as the first argument of the function after the pipe operator.
...
knitr::include_graphics(here::here("www", "img", "alpha.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "look_around.png"))
```
...
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics(here::here("www", "img", "regex_period.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "gender_race_ethnicity.png"))
```
...
Here was the text from our first image of this table:
...
See [this other case study](https://www.opencasestudies.org/ocs-bp-vaping-case-study/) for more details about how to write a function.
...
Great! Now let's apply our function to the `asian_sub_2017` data!
...
These images are called `asian_sub_2017_A`, `asian_sub_2017_B`, and `asian_sub_2017_C`.

Now we will apply our `make_rows()` function to the text from these images.
...
***
<details><summary> Click here to see what is incorrect. </summary>
...
First let's take a look of the text from the images we imported:
...
```{r}
latinx_sub_2017 <- stringr::str_c(latinx_sub_2017_A, latinx_sub_2017_B, latinx_sub_2017_C)
...
```{r, echo=FALSE}
save(latinx_sub_2017, file = here("www", "exercise", "dw_code1.rda"))
```
...
load(here("www", "exercise", "dw_code1.rda"))
```
...
If you completed the exercise, you would get an output like this:
...
```{r, echo=FALSE}
save(latinx_sub_2017, file = here("www", "exercise", "dw_code2.rda"))
```
...
load(here("www", "exercise", "dw_code2.rda"))
```
...
```{r, echo=FALSE}
save(latinx_sub_2017, file = here("www", "exercise", "dw_code3.rda"))
```
...
load(here("www", "exercise", "dw_code3.rda"))
```
...
### **Asian Subgroups 2018**
***
Recall that this was the page with the table of interest for the Asian subgroups with 2018 data (published in 2020):
...
Trial and error indicated that again dividing the table into multiple screenshots improved the text extraction. We saved the text from these images during the [**Data Import**] section as `asian_sub_2018_A` and `asian_sub_2018_B`.
...
Again, let collapse these together with `str_c()` function.

```{r}
asian_sub_2018 <-str_c(asian_sub_2018_A, asian_sub_2018_B)
```

Now we can apply our `make_rows()` function:
...
***
<details> <summary> Click here to see how this interactive table was created </summary>
...
In this case only a single image was needed. We previously imported the text form this image and called it `latinx_sub_2018`:
...
Now we can apply our `make_rows()` and `clean_table()` functions:
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img", "latinx_youth_excerpt.png"))
...
Now let's save the wrangled data so we can use it for later if we wanted to start again at this point. We can do so using the `save` function like so. We will save this data in a subdirectory called "wrangled" within the "data" directory of our working directory. 
We will also save a csv file as this is often useful to send our data to collaborators. To use the `write_csv()` function we need to save a different CSV file for each tibble.

```{r, eval=FALSE}
save(major_groups_long, asian_subgroups, latinx_subgroups, file = here::here("data", "wrangled", "wrangled_data.rda"))
readr::write_csv(major_groups_long, file = here::here("data", "wrangled", "wrangled_major_groups_data.csv"))
readr::write_csv(asian_subgroups, file = here::here("data", "wrangled", "wrangled_asian_subgroups_data.csv"))
readr::write_csv(latinx_subgroups, file = here::here("data", "wrangled", "wrangled_latinx_subgroups_data.csv"))

```

```{r, echo=FALSE}
save(major_groups_long, asian_subgroups, latinx_subgroups, file = here::here("www", "data", "wrangled", "wrangled_data.rda"))
readr::write_csv(major_groups_long, file = here::here("www", "data", "wrangled", "wrangled_major_groups_data.csv"))
readr::write_csv(asian_subgroups, file = here::here("www", "data", "wrangled", "wrangled_asian_subgroups_data.csv"))
readr::write_csv(latinx_subgroups, file = here::here("www", "data", "wrangled", "wrangled_latinx_subgroups_data.csv"))
...
quiz(caption = "",
  ...
  question("Which of the following are the main arguments in the `pivot_longer()` function? (Select all that apply)",
    answer("`names_from`", message = "`names_from` is a main argument in the `pivot_wider` function."),
    answer("values_from", message = "`values_from` is a main argument in the `pivot_wider` function."),
    answer("`cols`", correct = TRUE),
    answer("`names_to`", correct = TRUE),
    answer("`values_to`", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("What function from the `dplyr` package can we use to **append** tibbles one after another?",
    answer("`bind_cols()`"),
    answer("`inner_join()`"),
...
Suppose a student was manually entering the first five rows of the `iris` R built-in dataset. He ended up missing some decimal points. Also, there is a typo in the third row where he put "setosq" instead of "setosa" in the `Species` column. 

This is what the first five rows of `iris` should look like.

```{r}
head(iris, n = 5)
```

Write some code that corrects the errors he made.

```{r dw_exercise1-setup}
library(tidyverse)
library(magrittr)

exercise_iris <- head(iris, n = 5) %>%
  mutate(across(everything(), as.character)) %>%
  mutate(Sepal.Width = recode(Sepal.Width, "3.2" = "32", "3.6" = "36"),
         Sepal.Length = recode(Sepal.Length, "4.6" = "46"))

exercise_iris[3, 5] <- "setosq"
exercise_iris[5, 1] <- "5.0"
exercise_iris[2, 2] <- "3.0"
```

```{r dw_exercise1, exercise=TRUE, exercise.eval = TRUE}
# view the data frame that the student entered
exercise_iris

# fix the missing decimal points

# fix the typo in the "Species" column
```

```{r dw_exercise1-solution}
# one possible solution
exercise_iris %<>%
  mutate(across(.cols = -Species, 
                ~str_remove(string = ., pattern = "\\."))) %>%
  mutate(across(.cols = -Species, as.numeric)) %>%
  mutate(across(.cols = -Species, ~ . * 0.1)) %>%
  mutate(Species = recode(Species, "setosq" = "setosa"))
...
```{r dw_exercise2, exercise=TRUE, exercise.eval=TRUE}
wide_format
```
...
If you have been following along but stopped and are starting here you could load the wrangled data using the `OCSdata` package like so:

```{r, eval=FALSE}
# install.packages("devtools")
# devtools::install_github("opencasestudies/OCSdata")
load_wrangled_data("ocs-bp-youth-disconnection")
```
...
Then, you may load the wrangled data into your R environment using the following function:

```{r, eval=FALSE}
load_wrangled_data("ocs-bp-youth-disconnection")
```

If the package does not work for you, an RDA file (stands for R data) of the data can be found [here](https://github.com//opencasestudies/ocs-bp-youth-disconnection/tree/master/data/wrangled) or slightly more directly [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-disconnection/master/data/wrangled/wrangled_data.rda). Download this file and then place it in your current working directory within a subdirectory called "wrangled" within a subdirectory called "data" to copy and paste our code. We used an RStudio project and the [`here` package](https://github.com/jennybc/here_here) to navigate to the file more easily. 
...
include_graphics(here::here("www", "img", "Making_the_Connection_plot.png"))
```
...
So for this first plot we just want to recreate the plot in the report. We will use the `major_groups_long` tibble and we will filter for only the `"All"` values of the `Gender` column.
...
To add the figurines we can use icons from free resources like [font awesome](https://fontawesome.com/). See [here](https://commons.wikimedia.org/wiki/Category:Font_Awesome_4_icons) for a link to `svg` options. Or we could also take a screenshot of just the figurine from the original plot. However, it is useful to know how to create similar icons if we didn't have the original plot to work with so we will start with an example of using a font awesome icon. 
...
Now let's try using a screenshot of the original plot. You can find our screenshot in our `img` directory on our GitHub repository.

#### Using a screenshot:
```{r, out.width= "10%", eval=FALSE}
...
figurine <-image_read(here::here("www", "img", "figurine.png"))
figurine
```
...
```{r, echo=FALSE}
save(major_groups_long, file = here("www", "exercise", "dv_code1.rda"))
```
...
load(here("www", "exercise", "dv_code1.rda"))
```
...
question("Which one of the following arguments inside the `ggplot()` function can be used for defining what variables to be plotted on the x/y-axis, as well as choosing variables to color or group the plot by?",
    answer("geom", message = "Geoms define geometric objects. This can define the type of plot that is being plotted."),
...
* have points connected by lines
* have different colors representing IDs 1, 2, and 3
* have direct labels for each line (ID)
...
```{r dv_exercise1, exercise=TRUE, exercise.eval=TRUE}
weight
```
...
weight_plot <- 
  directlabels::direct.label(weight_plot, 
                             list(dl.trans(x = x + 0.2, y = y + 0), 
                                  "last.points"))
...
See this [link](https://vsp.pnnl.gov/help/vsample/Design_Trend_Mann_Kendall.htm) for information on using this test with missing data.
...
```{r,echo=FALSE, out.width="80%"}
 include_graphics(here::here("www", "img", "zscore.png"))
```
...
knitr::include_graphics(here::here("www", "img", "twosided.png"))
```
...
quiz(caption = "",
  question("Pooled cross-sectional data is longitudinal data from repeated measurement of the same people.",
    answer("True", message = "Panel data is longitudinal data from repeated measurement of the same people. Pooled cross-sectional data is data produced from repeated measurement of a population over time (the individuals in each sample are not necessarily the same individuals)."),
    answer("False", correct = TRUE, message = "Panel data is longitudinal data from repeated measurement of the same people. Pooled cross-sectional data is data produced from repeated measurement of a population over time (the individuals in each sample are not necessarily the same individuals)."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("Both the Mann-Kendall trend test and simple linear regression are parametric tests.",
    answer("True", message = "The Mann-Kendall trend test is a nonparametric test. Simple linear regression is a parametric test."),
    answer("False", correct = TRUE, message = "The Mann-Kendall trend test is a nonparametric test. Simple linear regression is a parametric test."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
...
To learn more about importing and wrangling PDFs using the `pdftools` package see this [case study](https://opencasestudies.github.io/ocs-bp-rural-and-urban-obesity/) and this [case study](https://opencasestudies.github.io/ocs-bp-diet/). 
...
<script type='text/javascript' id='clustrmaps' src='//cdn.clustrmaps.com/map_v2.js?cl=080808&w=a&t=tt&d=O0N6_xyVRrI9LRa4Y29rluaIuSuY2k5TdZXLWTjco2U&co=ffffff&cmo=3acc3a&cmn=ff5353&ct=808080'></script>