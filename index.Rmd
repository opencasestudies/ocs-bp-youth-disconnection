...
There does not appear to be a change in the trend. However, it's important to note that we only have `r length(dis_long %>% filter(Gender == "Gender_Total", Race == "Native American") %>% pull(Rate))` observations.
...
`r (dis_long%>% filter(Gender == "Gender_Total", Race == "Native American") %>% lm(Rate ~ Year, data = .))[["coefficients"]]["Year"]`
...
asian_subgroups %>%
  ...
  group_by(Race) %>%
  mutate(rank_all = sub_rank[Gender == "Gender_Total"]) %>%
  ungroup() %>%
