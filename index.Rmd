...
**This is a another real example of the above**
...
We will expand beyond this [repoart](https://ssrc-static.s3.amazonaws.com/moa/Making%20the%20Connection.pdf) to create visualizations of percent disconnection of more specific groups of youths (gender, ethnic groups etc.) graphed together over time to allow for easier comparison.
...
We have repeated cross-sectional data.
...
At specific time-points, data is collected from a sample of the population. The individuals in each sample are not necessarily the same individuals. This separates repeated cross-sectional data from longitudinal data, which is data from repeated measurement of the same people.   
...
The Mann-Kendall trend test tests whether there is a monotonic association, an association that does not increase or decrease but remains static across a dimension.
...
We can accomplish this with the `Kendall::MannKendall()` function. The `Kendall::MannKendall()` accepts a vector of data for which a trend may be observed.    
...
As we can see, there is a large amount of uncertainty around the OLS regression line. 
